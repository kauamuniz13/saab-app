...
const zones = {
  ...
  'CONTAINER 36': 'CONTAINER_36',
  'CONTAINER 33': 'CONTAINER_33',
  'CONTAINER 32': 'CONTAINER_32',
  'CONTAINER 31': 'CONTAINER_31',
  'SECOS': 'SECOS',
  'BEBIDAS': 'BEBIDAS'
};
...
const regex = /^(\d+)\s+(caixa|caixas|garrafa|garrafas|unidade|unidades)\s+(.+)$/i;

for (let line of lines) {
  line = line.trim();
  if (!line || ['nassif', 'saab'].includes(line.toLowerCase())) {
    continue;
  }
  ...
  if (match) {
    ...
    items.push({ zone: current_zone, qty, unit, name });
  } else {
    console.warn('Unparsed:', line);
  }
}

fs.writeFileSync('seed_data.json', JSON.stringify(items, null, 2));
